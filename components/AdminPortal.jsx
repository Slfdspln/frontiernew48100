...
export default function AdminPortal() {
  ...
  const [authStatus, setAuthStatus] = useState({ loading: true, isAdmin: null });
  
  useEffect(() => {
    // Check if user is logged in (same check as resident portal)
    const checkAdminAuth = async () => {
      try {
        const r = await fetch('/api/auth/reverify', { method: 'POST' });
        const isLoggedIn = r.ok && (await r.json()).isResident;
        setAuthStatus({ loading: false, isAdmin: isLoggedIn });
      } catch {
  ...
  if (!authStatus.isAdmin) return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="text-center space-y-4 max-w-md mx-auto px-6">
        <div className="text-red-600 text-lg">🔐 Admin Login Required</div>
        ...
        <div className="space-y-2">
          <button 
            ...
            className="block mx-auto px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors"
          >
            Go to Login
          </button>
          <p className="text-sm text-gray-500">
            Regular credentials work for now. Proper admin login will be added later.
          </p>
        </div>
      </div>
    </div>
